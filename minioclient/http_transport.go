...
func readPEMCertKey(filename string) (certs []byte, key []byte, err error) {
	...
	for {
		...
		case strings.HasSuffix(block.Type, "PRIVATE KEY"):
			if key != nil {
				return nil, nil, errors.New("error loading TLS cert and key from : more than one private key found " + filename)
			}
			key = pem.EncodeToMemory(block)
		default:
			return nil, nil, errors.New("error loading TLS cert and key from %v: unknown block type %v found" + filename + block.Type)
		}
	}
...
func Transport(opts TransportOptions) (http.RoundTripper, error) {
	...
	if opts.RootCertFilenames != nil {
		...
		for _, filename := range opts.RootCertFilenames {
			if filename == "" {
				return nil, errors.New("empty filename for root certificate supplied")
			}
			...
			if ok := pool.AppendCertsFromPEM(b); !ok {
				return nil, errors.New("cannot parse root certificate from " + filename)
			}
		}
